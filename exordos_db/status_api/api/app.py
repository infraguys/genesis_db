...
from exordos_db.status_api.api import routes as app_routes
from exordos_db.status_api.api import versions
from exordos_db import version

skip_sdk_endpoints = ["/specifications/3.0.3"]
...
def get_openapi_engine():
    openapi_engine = openapi_engines.OpenApiEngine(
        info=openapi_structures.OpenApiInfo(
            title=f"Exordos DB {versions.API_VERSION_v1} Status API",
            version=version.version_info,
            description=f"OpenAPI - Exordos DB {versions.API_VERSION_v1}",
        ),
